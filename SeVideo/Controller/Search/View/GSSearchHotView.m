...
@implementation GSSearchHotView
...
- (void)initDefaultConfign{
    ...
    _maragin_x = _maragin_y = DEFAULT_INTERVAL;
    _titleLabHeight = 30;
    _radius = 6;
...
- (CGRect)setSignView:(NSArray *)dataAr andTitle:(NSString *)titleStr andFrame:(CGRect)frame andGroupId:(NSInteger)groupId{
    ...
    for (NSInteger i = 0; i < dataAr.count; ++i) {
        @autoreleasepool {
            ...
            [but setTitle:[GSTools stringEncoding:dataAr[i]] forState:UIControlStateNormal];
...
- (void)contentSignalWith:(UIButton *)sender{
    ...
    //传递当前选中的Value
    if ([self.delegate respondsToSelector:@selector(gs_selectCurrentValueWith:index:groupId:)]) {
        [self.delegate gs_selectCurrentValueWith:self.dataSourceArr[sender.tag / 100][sender.tag % 100 - 1] index:sender.tag % 100 - 1 groupId:sender.tag / 100];
    }
}
